...
export class TeachersService {
    ...
    async loggedInTeachers(payload: IUser) {
        ...
        const loggedInTime = payload.last_login as unknown as Date;
        ...
        if (loggedInTime.getDay() <= 5) {
            ...
            if (loggedInTime.getHours() >= schoolStarts && loggedInTime.getHours() <= schoolEnds) {
                ...
                stats.statistics.logged_in_teachers = stats.statistics.logged_in_teachers ? stats.statistics.logged_in_teachers : [];
                stats.statistics.logged_in_teachers.push({
                    id: payload.id,
    ...
    async resetLoggedInTeachers() {
        ...
        if (now.getDay() <= 5) {
            ...
            schools.forEach(async (school) => {
                ...
                let loggedInTeachers = stats.statistics.logged_in_teachers;
                ...
                loggedInTeachers.forEach(teacher => {
